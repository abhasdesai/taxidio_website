...
class Home extends Front_Controller {
	...
	function index()
	{
		...
		if($this->config->item('base_url')=='http://192.168.0.123/taxidio/')
		{
			$data['blogs'] = array();
		}
		else
...
//echo "<pre>";print_r($data);die;
				...
				else
				{
						...
						foreach($attraction_decode as $k=>$v)
						{
							//echo "<pre>";print_r($v['properties']['attractionid']);die;
							$note_data = $this->Home_fm->getNotes($userid = $this->session->userdata('fuserid'), $v['properties']['attractionid'],$v['properties']['cityid']);

							$v['properties']['notes'] = (!empty($note_data)) ? substr($note_data['note'], 0, 1) : '';
					    ...
					    /*if($this->session->userdata('fuserid') == '250')
						{
							echo '<pre>';print_r( $data['filestore']);die;
						}*/
