...
class Attraction_fm extends CI_Model
{
	...
	function getUsersMultiCountryRecommendations($cityfile,$uniqueid)
	{
		...
		if(!file_exists(FCPATH.'userfiles/multicountries/'.$this->session->userdata('randomstring').'/'.$uniqueid.'/'.$cityfile))
		{
			...
			{
				$ids=$this->getIDS($inputdecode['tags']);
				$this->getSelectedAttractionsMultiCountry($ids,$cityfile,$uniqueid,0);

			}
			else
			{
				$this->writeAllUserAttractionMultiCountry($cityfile,$uniqueid,0);
			}
			return 1;
		}
		return 2;
	...
	function writeAllUserAttractionMultiCountry($city_id,$uniqueid,$isnew)
	{
		$c=0;
		$key2array=array();
		$key2key='';
		...
		$file=fopen(FCPATH.'userfiles/multicountries/'.$this->session->userdata('randomstring').'/'.$uniqueid.'/'.$city_id,'w');
		fwrite($file,json_encode($attraction_decode));
		fclose($file);
	}
	...
	function getSelectedAttractionsMultiCountry($ids,$city_id,$uniqueid,$isnew)
	{
		$c=0;
		$key2array=array();
		$key2key='';
	...
	function makeFileForThisCityMulti($cityfile,$uniqueid)
	{
		...
		if(!file_exists(FCPATH.'userfiles/multicountries/'.$this->session->userdata('randomstring').'/'.$uniqueid.'/'.$cityfile))
		{
			...
			{
				$ids=$this->getIDS($inputdecode['tags']);
				$this->getSelectedAttractionsMultiCountry($ids,$cityfile,$uniqueid,1);

			}
			else
			{
				$this->writeAllUserAttractionMultiCountry($cityfile,$uniqueid,1);
			}
			return 1;
		}
		return 2;
