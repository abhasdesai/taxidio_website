...
class Relaxationspa_m extends CI_Model {
	...
	function add() {
		...
		$ras_name=explode(',',$_POST['ras_name']);
		$zipcode='';
		...
		$data = array(
			'ras_name' => $ras_name[0],
			'ras_description' => $_POST['ras_description'],
			'ras_address' => $_POST['ras_address'],
			'ras_lat' => trim(substr($_POST['ras_lat'],0,16)),
			'ras_long' => trim(substr($_POST['ras_long'],0,16)),
			'ras_contact' => $_POST['ras_contact'],
			'ras_website' => $_POST['ras_website'],
			'ras_timing' => $_POST['ras_timing'],
		...
		{		
			...
			if (!$this->upload->do_upload('image'))
			{
				...
				redirect('admins/city/Relaxationspa/add/'.$_POST['city_id']);
			}
			else
			{
		...
		$this->db->insert('tbl_city_relaxationspa', $data);

		$Qt=$this->db->query('select id,tag_name from tbl_tag_master');
		$qttags=$Qt->result_array();
		$this->writeAttractionsInFile($_POST['city_id'],trim(substr($_POST['ras_long'],0,16)),trim(substr($_POST['ras_lat'],0,16)),$qttags);
	...
	function edit() {
	  ...
	  if(isset($_POST['zipcode']) && $_POST['zipcode']!='')
	  {
		...
		$data = array(
			...
			'ras_description' => $_POST['ras_description'],
			'ras_address' => $_POST['ras_address'],
			'ras_lat' => substr($_POST['ras_lat'],0,20),
			'ras_long' => substr($_POST['ras_long'],0,20),
			'ras_contact' => $_POST['ras_contact'],
			'ras_website' => $_POST['ras_website'],
			'ras_timing' => $_POST['ras_timing'],
			'known_for'=>$known_for,
		...
		{		
			...
			if (!$this->upload->do_upload('image'))
			{
				...
				$error = array('warning' =>  $this->upload->display_errors());
				$this->session->set_flashdata('error', ($error['warning']));
				redirect('admins/city/Relaxationspa/edit/'.$_POST['id']);
			}
			else
			{
				$image = $this->upload->data();
		...
		$this->db->update('tbl_city_relaxationspa', $data);
	...
	{
		$Q=$this->db->query('select image from tbl_city_relaxationspa where id="'.$id.'"');
		$data=$Q->row_array();
		if($data['image']!='')
		{
	...
	function getDetailsById($id) {
		$Q = $this->db->get_where('tbl_city_relaxationspa', array('id' => $id));
		return $Q->row_array();
	}

	function check_continent($continent_name) {
		$this->db->select('id');
		$Q = $this->db->get_where('tbl_city_relaxationspa', array('id !=' => $_POST['id'], 'continent_name' => $continent_name));
		if ($Q->num_rows() > 0) {
	...
	function delete($id) {
		$Q=$this->db->query('select city_id,country_id,zipcode,ras_lat,ras_long from tbl_city_relaxationspa where id="'.$id.'"');
		$data=$Q->row_array();
		...
		$Qt=$this->db->query('select id,tag_name from tbl_tag_master');
		$qttags=$Qt->result_array();
		$this->writeAttractionsInFile($data['city_id'],$data['ras_long'],$data['ras_lat'],$qttags);
