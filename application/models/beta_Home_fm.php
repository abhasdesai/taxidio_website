...
class Home_fm extends CI_Model {
	...
	{
		$Q=$this->db->query('select slug from tbl_country_master where rome2rio_name ="'.trim($countryrome2rioname).'"');
		return $Q->row_array();
	}
	...
	function CountriesWithNoTags()
	{
		...
		/*Developer*/
		$this->db->select('tbl_city_master.id,city_name,tbl_city_master.slug,tbl_city_master.latitude,tbl_city_master.longitude,tbl_city_master.rome2rio_name,tbl_city_master.country_id,tbl_country_master.country_name,tbl_country_master.latitude as countrylatitude,tbl_country_master.longitude as countrylongitude,tbl_country_master.continent_id,tbl_country_master.slug,tbl_country_master.rome2rio_name as rome2rio_country_name,tbl_city_master.latitude as citylatitude,tbl_city_master.longitude as citylongitude,country_conclusion,total_days,budget_hotel_per_night_from,budget_hotel_per_night_to,tbl_city_master.max_zipcode,tbl_city_master.slug as cityslug,code,countrycode,countryimage,cityimage,UNIX_TIMESTAMP() as uniqueid',FALSE);
		...
		{
			$this->db->where('tbl_country_master.rome2rio_name',trim($countryrome2rioname));
			if (isset($_GET['days']) && $_GET['days'] != '') 
			{

				$plus = substr($_GET['days'], -1);
				if ($plus == '+') 
				{
					$days = rtrim($_GET['days'], '+');
					$startday = $_GET['days'] - 2;
					$this->db->where('tbl_city_master.total_days >=', $startday,FALSE);
		...
		if (isset($_GET['weather']) && $_GET['weather'] != '') 
		{
			...
			if ($plus == '+') 
			{
				$weather = rtrim($_GET['days'], '+');
				$this->db->where('weather_avg >=', $weather,FALSE);
				$this->db->where('month_id',$month,FALSE);

			}
			else
			{
				$weather = explode('-',$_GET['weather']);
				if(count($weather)==4)
				{
				...
				$whereweather = "(weather_avg BETWEEN $startweather AND $endweather)";
		...
		if (isset($_GET['budget']) && $_GET['budget'] != '') 
		{
			...
			else
			{
				...
				$startbudget=$budget[0];
				$endbudget=$budget[1];
				$start_budget = round($startbudget-30);
				$end_budget = round($endbudget+30);

				$wherebudget = "($start_budget BETWEEN budget_hotel_per_night_from AND budget_hotel_per_night_to or $end_budget BETWEEN budget_hotel_per_night_from AND budget_hotel_per_night_to)";

				//$this->db->where('budget_hotel_per_night_from >=', $start_budget,FALSE);
				//$this->db->where('budget_hotel_per_night_to <=', $end_budget,FALSE);
	...
	function InternationalNoTags($data)
	{
		$country_group = array();
				...
				if (isset($_GET['days']) && $_GET['days'] != '') 
				{
					...
					{
						$days = rtrim($_GET['days'], '+');
						$startday = (int)($_GET['days'] - 2);
						$endday=0;
					} 
					else 
					{
						$startday = (int)($_GET['days'] - 2);
						$endday = (int)($_GET['days'] + 2);
				...
				foreach($country_group as $key=>$list)
				{
					...
					{
						$totaldays+=$innerlist['total_days'];
					}
					//$country_group[$key]['totaldays']=$totaldays;
	...
	function selectCountries($data) {
		...
		$plus = substr($_GET['traveltime'], -1);
		if ($plus == '+') {
			$approx = 0.20 * $_GET['traveltime'];
			$starttime = round(rtrim($_GET['traveltime'], '+')-$approx);
			$endtime = 0;
	...
	function getShortestDistance1($key, $start_city, $end_city) {
		$url = 'http://free.rome2rio.com/api/1.2/json/Search?key=gQIcwUKF&query&oName=' . urlencode($start_city) . '&dName=' . urlencode($end_city) . '';
	...
	function getShortestDistance($key, $start_city, $end_city) {
		...
		$url = 'http://free.rome2rio.com/api/1.2/json/Search?key=xa3wFHMZ&query&oName=' . urlencode($start_city) . '&dName=' . urlencode($end_city) . '';
		$ch = curl_init();
	...
	function getMultiCountries() 
	{
		...
		if(is_dir(FCPATH.'userfiles/multicountries/'.$this->session->userdata('randomstring')))
		{
			$files = glob(FCPATH.'userfiles/multicountries/'.$this->session->userdata('randomstring').'/*');
	...
	function MultiCountriesWithTags()
	{
		...
		$this->db->select('tbl_city_attraction_log.*,tbl_city_master.id,city_name,tbl_city_master.slug,tbl_city_master.latitude,tbl_city_master.longitude,tbl_city_master.rome2rio_name,tbl_country_master.country_name,tbl_country_master.latitude as countrylatitude,tbl_country_master.longitude as countrylongitude,tbl_country_master.continent_id,tbl_country_master.slug,tbl_country_master.rome2rio_name as rome2rio_country_name,tbl_city_master.latitude as citylatitude,tbl_city_master.longitude as citylongitude,country_conclusion,tbl_city_master.continent_id,tbl_country_master.rome2rio_name as country_rome2rio_name,tbl_country_master.id as countryid,tbl_city_master.max_zipcode,tbl_city_master.slug as cityslug,code,countrycode,countryimage,cityimage,UNIX_TIMESTAMP() as uniqueid',FALSE);
		...
		if(isset($_GET['isdomestic']) && $_GET['isdomestic']==1)
		{
			$this->db->where('tbl_country_master.rome2rio_name',$countryrome2rioname);
		}
		else
		{
			$this->db->where('tbl_country_master.rome2rio_name !=',$countryrome2rioname);
	...
	function selectMultiCountriesWithNoTags($data,$startday,$endday,$originalArray)
	{
		...
		{	
			...
			{	
				...
				else
				{
					...
					$url = 'http://free.rome2rio.com/api/1.2/json/Search?key=xa3wFHMZ&query&oName=' . urlencode($_GET['start_city']) . '&dName=' . urlencode($l['rome2rio_country_name']) . '';
					$ch = curl_init();
					curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
					curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
					curl_setopt($ch, CURLOPT_URL,$url);
					...
					curl_close($ch);
					$json = json_decode($result, true);
					$neededArray[$k]['actualtime']=$json['routes'][0]['duration'];
					$hours = floor($json['routes'][0]['duration'] / 60);
					$minutes = $json['routes'][0]['duration'] % 60;
						
					$tempstorage[]=array(
							'country_id'=>$l['country_id'],
							'distance'=>$hours . ' Hrs ' . $minutes . ' Mins',
							'actualtime'=>$json['routes'][0]['duration']
					);
		...
		foreach($new as $finalkey=>$finallist)
		{
			...
			else
			{
				...
				{
					$totalsum+=$sumlist['totalcountrytime'];
				}
				if($endday==0)
				{
	...
	function getUserRecommededAttractionsForNewCity($cityfile,$uniqueid)
	{
		if(!file_exists(FCPATH.'userfiles/files/'.$this->session->userdata('randomstring').'/'.$uniqueid.'/'.$cityfile))
		{
			...
			$file=fopen(FCPATH.'userfiles/files/'.$this->session->userdata('randomstring').'/'.$city_id,'w');
	...
	function getCityDetails($attractionid)
	{
		...
		if($_POST['category']>4 && $_POST['category']<1)
		{
			$arr=array(
	...
	{
		$this->db->select('tbl_city_paidattractions.id,image,attraction_name,attraction_details,attraction_address,attraction_contact,attraction_website,attraction_public_transport,attraction_timing,attraction_time_required,attraction_wait_time,attraction_buy_ticket,tag_name');
	...
	function getSuggestedCities($q) 
	{
		...
		$this->db->join('tbl_country_master','tbl_country_master.id=tbl_city_master.country_id');
		$this->db->where('tbl_city_master.id in (select city_id from tbl_city_paidattractions where city_id=tbl_city_master.id)');
		$where="(`city_name` LIKE '%".$q."%' ESCAPE '!' OR `country_name` LIKE '%".$q."%' ESCAPE '!')";
		$this->db->where($where);
		$query = $this->db->get();
		if($query->num_rows() > 0)
		{
	...
	function getCountry($slug){
		$Q=$this->db->query('select id,country_name,country_capital,country_neighbours, country_conclusion from tbl_country_master where slug="'.$slug.'"');
		return $Q->row_array(); 
	}
