...
class Travelguidepackage_m extends CI_Model {


    function getAllTravelGuidePackages($id='')
    {
       $data=array();
       $query = 'select * from tbl_travel_guide_package';
    ...
    function add($data)
    {
		$this->db->insert('tbl_travel_guide_package', $data);
    }
    function edit($data = array(), $id = '') 
    {
		$this->db->where('id', $id);
		$this->db->update('tbl_travel_guide_package', $data);
	}
	function delete($id) 
	{

		$Q=$this->db->query('select id from tbl_travel_guide_package where id="'.$id.'"');
		if($Q->num_rows()>0)
		{
			$data = array('status'=>0);
			$this->db->where('id', $id);
		...
		else
		{
			...
			redirect('admins/TravelGuidePackages');
