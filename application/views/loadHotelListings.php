...
<div class="titles">
    ...
    <?php foreach ($hotels as $list) { 
          ...
          if(isset($checkin) && $checkin!='')
          {
            ...
            $myrate=0;
            $rates=array($list['SEASON1START'],$list['SEASON2START'],$list['SEASON3START'],$list['SEASON4START'],$list['SEASON5START']);  
             for($r=0;$r<count($rates);$r++)
            {

               if(isset($rates[$r+1]) && $rates[$r+1]!='')
               {
                  if(strtotime($rates[$r])>=strtotime($checkin) && strtotime($checkin)< strtotime($rates[$r+1]) )
                  {
                     //echo $checkin.'>='.$rates[$r].'-->'.$checkin.'<'.$rates[$r+1].'<br/>';
                      $myrate=$r;
                      break;
                  }
               }
            ...
            if($myrate==0){
              $myrate=1;
              $level='LRA_S'.($myrate).'_RT1_SGL';
            }
            else
            {
              $level='LRA_S'.($myrate+1).'_RT1_SGL';  
            }
