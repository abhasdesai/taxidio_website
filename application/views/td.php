 ...
 <div class="inner-mainview container">
    ...
    <div class="row">
          ...
          <?php /* ?>
           ...
           <li class="ui-state-default <?php if($c==1){ echo 'active-li'; } ?>"><a href="<?php echo site_url('home/td').'/'.md5($list['id']) ?>"><?php echo $list['city_name']; ?></a>
            <span class="destination-time">5hrs 13min<br/><i class="fa fa-fighter-jet" aria-hidden="true"></i></span> 
            </li>
...
<script>
  ...
  $(document).ready(function(){
          ...
          // This will let you use the .remove() function later on
            ...
            waypoints2=JSON.parse('<?php echo $waypoints; ?>');
            var c=0;
            var f=0;
            ...
            waypoints2.forEach(function(obj) 
            {
                  if(c==0)
                  {
                     directions.setOrigin([parseFloat(obj.alongitude),parseFloat(obj.alatitude)]);
                     directions.addWaypoint(f,[parseFloat(obj.alongitude),parseFloat(obj.alatitude)]);
                     f++;
                  }
                  else if(c==end-1)
                  {
                      directions.setDestination([parseFloat(obj.alongitude),parseFloat(obj.alatitude)]);
                  }
                  else
                  {
                     directions.addWaypoint(f,[parseFloat(obj.alongitude),parseFloat(obj.alatitude)]);
         ...
         map.addControl(directions);
            ...
            directions.setOrigin([73.18121870000004,22.30718]);
            directions.addWaypoint(0, [73.18121870000004, 22.30]);
            directions.addWaypoint(1, [72.87765590000004, 19.07598]);
            directions.addWaypoint(2, [74.12399600000003, 15.299326]);
