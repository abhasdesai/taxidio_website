...
<script>
  ...
  $(document).ready(function(){
          ...
          // This will let you use the .remove() function later on
           ...
           /* directions.setOrigin([72.89061100000004,19.1030793]);
            directions.addWaypoint(0, [72.86561440000003,19.0895595]);
            directions.addWaypoint(1, [72.84697559999995,19.1007158]);
            directions.addWaypoint(2, [72.83441249999998,19.0800297]);
            directions.setDestination([72.83441249999998,19.0800297]);
            ...
            */
            ...
            var f=0;
            var end=0;
            filestore.forEach(function(marker, i){ 
              ...
              if(marker.isselected==1 && marker.properties.isplace==1)
               {
                  ...
                  if(c==0)
                  {
                     directions.setOrigin([parseFloat(marker.geometry.coordinates[0]),parseFloat(marker.geometry.coordinates[1])]);
                     directions.addWaypoint(f,[parseFloat(marker.geometry.coordinates[0]),parseFloat(marker.geometry.coordinates[1])]);
                     f++;
                  }
                  else if(c==end-1)
                  {
                     directions.setDestination([parseFloat(marker.geometry.coordinates[0]),parseFloat(marker.geometry.coordinates[1])]);
                     return false;
                  }
                  else
                  {
           ...
           /* waypoints2=monthArrayr;
            var c=0;
            var f=0;
            var end=parseInt(waypoints2.length); 
            waypoints2.forEach(function(obj) 
            {
                  if(c==0)
                  {
                     directions.setOrigin([parseFloat(obj.longitude),parseFloat(obj.latitude)]);
                     directions.addWaypoint(f,[parseFloat(obj.longitude),parseFloat(obj.latitude)]);
                     f++;
                  }
                  else if(c==end-1)
                  {
                      directions.setDestination([parseFloat(obj.longitude),parseFloat(obj.latitude)]);
                  }
                  else
                  {
                     directions.addWaypoint(f,[parseFloat(obj.longitude),parseFloat(obj.latitude)]);
