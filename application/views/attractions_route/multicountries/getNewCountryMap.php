 <div class="row">
          ...
          <ul id="sortable">
            ...
            <span class="delete-city" onClick="removeCityFromMultiCountry('<?php echo string_encode($combination) ?>')"><a href="javascript:void(0);"> 
            ...
            <li class="ui-state-default nodrag" id="lastdragli"><a href="javascript:void(0);" onClick="addCityInMultiCocuntry('<?php echo string_encode($country) ?>')">Add New</a>
           </li>
              ...
              <div class="sidebar col-md-4">
                 ...
                 <div class="col-md-12 inner-links">
                  <div class="text-left">
                    ...
                    <a id="showmultiall" idattr="<?php echo md5($attractioncities[0]['id']); ?>" class="link-button modal-link" href="javascript:void(0);"><i class="fa fa-plus-circle" aria-hidden="true"></i>View All</a>
...
<script>
  ...
  $(document).ready(function(){
            ...
            function buildLocationList(data) {
              for (i = 0; i < data.features.length; i++) {
                ...
                if(currentFeature.tempremoved==1)
                {
                    ...
                    linkadd.innerHTML = '<i class="fa fa-plus" aria-hidden="true" onclick="addNewMultiAttraction(\'' + prop.attractionid + '\',\'' + prop.cityid + '\')"></i>';    
                }
                ...
                if(currentFeature.tempremoved==0)
                {
                    ...
                    linkdel.innerHTML = '<i class="fa fa-trash-o" aria-hidden="true" onclick="deleteNewMultiAttraction(\'' + prop.attractionid + '\',\'' + prop.cityid + '\')"></i>';
                }       
          ...
          var secretid='<?php $secretid=""; if(isset($countryid_encrypt) && $countryid_encrypt!=""){ echo $countryid_encrypt; } ?>'; 
          //alert(secretid);
          if(secretid!='')
          {
              if($("#hotellink").length)
              {
                 var url='<?php echo site_url("showHotelsOfMultiCountries") ?>/'+secretid;
                 $("#hotellink").attr('href',url);
              }
          }
         
          var cityimage='<?php echo $cityimage ?>';
          $(".mainview").attr("style","background:url('"+cityimage+"')"); 
          var countynm='<?php echo $country_name; ?>';
           var countyconclusion='<?php echo $countryconclusion ?>';
          $(".country-name").html('Explore '+countynm);
           $("#pcountry-name").html(countyconclusion);

          var countrybanner='<?php if(isset($countrybanner) && $countrybanner!=""){ echo $countrybanner; } else { echo "0"; } ?>';
          if(countrybanner!=0)
          {
            $("#countryimg").attr("src",countrybanner); 
          }
          var citypostid='<?php echo $citypostid ?>';
          $("#citypostid").val(citypostid);
          var attrurl='<?php echo $attrurl ?>';
          $("#aattr").attr('href',attrurl);
...
$(document).ready(function(){
    ...
    var countrynm="<?php if(isset($countrynm)){ echo $countrynm['country_name']; } ?>";
    if(countrynm!='')
    {
      $("#countrynameexp").html('Explore '+countrynm);
    }
    var options='<?php echo $options ?>';
    if(options==1)
    {
       $(".mainform").html('<h3>we do not have any other city for this country</h3>');
    }
    else
    {
        $(".mainform").html('<div class="row"><div class="control-group"><label class="control-label" for="location">City Name1:</label><div class="controls"><select id="countrycityselected" name="cityname" type="text" class="form-control" placeholder="" class="input-medium" required="">'+options+'</select><input id="countryofcity" name="countryofcity" type="hidden" class="form-control" placeholder="" class="input-medium" value="<?php echo $country; ?>"></div></div></div><div class="row"><div class="control-group"><label class="control-label" for="Save"></label><div class="controls"><input type="submit" class="action-button shadow animate blue" id="btnaddcity" value="Add" /></div></div></div>');
    }
