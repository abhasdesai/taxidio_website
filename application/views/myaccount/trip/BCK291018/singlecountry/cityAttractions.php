...
<?php } ?>
 ...
 <div class="inner-mainview container">
   ...
   <div class="row">
          ...
          <ul id="sortable" class="savedSearchcity-x">

            <?php $c=0; foreach($attractioncities as $list){ $c++; ?>

            <li id="drag-x-<?php echo $c-1; ?>" class="ui-state-default drag-x <?php if($c==1){ echo 'active-li'; } ?>">
            
            <a href="javascript:void(0);" onclick="funOpen('<?php echo md5($list['id']) ?>')"><?php echo $list['city_name']; ?></a>
           ...
           <?php if($filenm!=$list['id']){ ?>
               <span class="delete-city" onClick="removeExtraCityFromSave('<?php echo $list['cityid'] ?>')"><a href="javascript:void(0);">
...
<div id="bindTab">
	...
	<?php }?>
        ...
        <div class="sidebar col-md-4 scrollbar-inner">
          <div id='listings' class='listings searchsavedbar'></div>
            </div>
		...
		<div class="col-md-12 inner-links">
			<div class="text-left">
			<div class="col-md-4 attractions-btn">
			  <a class="link-button modal-link" data-toggle="modal" id="singlemodelsaved" href="#"><i class="fa fa-plus-circle" aria-hidden="true"></i>Add New Location</a>
			  </div>
			  <div class="col-md-4 attractions-btn">
			  <a id="showallSearchedSaved" idattr="<?php echo md5($attractioncities[0]['id']); ?>" class="link-button modal-link" href="javascript:void(0);"><i class="fa fa-plus-circle" aria-hidden="true"></i>View All</a>
			  </div>

			  <div class="col-md-4 attractions-btn">
			   <?php if($this->session->userdata('fuserid')!=''){ ?>
				<a href="<?php echo site_url('update-searched-itinerary').'/'.string_encode($filenm.'-'.$itineraryid); ?>" class="link-button modal-link">Save<i class="fa fa-map-signs" aria-hidden="true"></i></a>
			  <?php } else { ?>
  ...
  <div class="row button-collections">
    ...
    <?php $param=urlencode($cityname).'/'.$longitude.'/'.$latitude ?>
      <ul class="button-ul">
        ...
        <li><a href="<?php echo site_url('hotelListsForsearchedCity').'/'.$countryid_encrypt; ?>" class="link-button" id="hotellink">Hotel Bookings<i class="fa fa-hand-o-right" aria-hidden="true"></i></a></li>
         ...
         <ul class="othercities-ul single-item">
            ...
            <?php foreach($othercities as $list){ ?>
                ...
                <li class="othercities-li">
                  <?php if(isset($list['cityimage']) && !empty($list['cityimage'])){ ?>
                   <img src="<?php echo site_url('userfiles/cities/small').'/'.$list['cityimage'] ?>"/>
                   <?php }else{ ?>
                   <img src="<?php echo site_url('assets/images/image300.jpg'); ?>" width="300" height="169"/>
                   <?php } ?>
                   ...
                   <p class="addcityp">
                   <a href="javascript:void(0);" id="<?php echo $list['cityid'] ?>" class="addsearchcitysaved link-button">Add</a>
                   </p>
                </li>
...
<script>
            ...
            function buildLocationList(data) {
              for (i = 0; i < data.features.length; i++) {
                ...
                if(currentFeature.tempremoved==1)
                {
                    ...
                    linkadd.innerHTML = '<i class="fa fa-plus" aria-hidden="true" onclick="addMainAttractionSingleSaved(\'' + prop.attractionid + '\',\'' + prop.cityid + '\')"></i>';
                }
                else if(currentFeature.tempremoved==0)
                {
                    ...
                    linkdel.innerHTML = '<i class="fa fa-trash-o" aria-hidden="true" onclick="deleteMainAttractionSingleSaved(\'' + prop.attractionid + '\',\'' + prop.cityid + '\')"></i>';
                }
  ...
  function funOpen(id)
  {
     ...
     $.ajax({
          type:'POST',
          url:'<?php echo site_url("getUserAttractionsOfSingleCountrySaved") ?>',
          data:'id='+id+'&iti='+$('#iti').val(),
          beforeSend: function(){
             $.LoadingOverlay("show");
          },
          complete: function(){
...
<div id="mapModal" class="modal fade" role="dialog">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <?php echo form_open('',array('id'=>'addNewActivitySearchedFormSaved')); ?>
				...
				<?php foreach($packages as $k=>$p){?>
					<div class="col-sm-4">
						...
						<div class="package text-center">
							...
							<div class="package-name"><?= $p->package_name?></div>
							<div class="package-name"><?= $p->package_qty?></div>
							<div class="package-name"><i class="fa fa-dollar"></i><?= $p->package_price?></div>
