			...
			that.form.find('input[type="file"]').change(function(){
				...
				if(that.options.processInline){			
					...
					$.ajax({
						...
						var checkstatus=JSON.parse(data);
						if(checkstatus.status=='failed')
						{
							$("#error-msg").show();
							$("#error-msg").html('something went wrong, most likely file is to large for upload or it does not have valid extension.')
							$(".cropControlUpload").show();
							that.hideLoader();
						}
						else
						{
							$("#image_name").val(JSON.parse(data).image_name);
